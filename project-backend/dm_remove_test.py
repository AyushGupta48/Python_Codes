...
def test_http_invalid_token(clear_data_http):
    ...
    response_list = requests.delete(
        f"{url}dm/remove/v1", json=list_payload).json()

    assert response_list.get('code') == 403


def test_http_user_not_creator(clear_data_http):
    payload1 = {
    ...
    response_remove = requests.delete(
        f"{url}dm/remove/v1", json=remove_payload).json()

    assert response_remove.get('code') == 403
...
def test_http_invalid_dm_id(clear_data_http):
    ...
    assert response_remove.get('code') == 400


def test_http_successful_remove(clear_data_http):
    payload1 = {
        "email": "valid@gmail.com",
        "password": "aBc293",
        "name_first": "Daniel",
    ...
    requests.delete(f"{url}dm/remove/v1", json=remove_payload).json()
